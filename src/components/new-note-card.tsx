...
export function NewNoteCard({ onNoteCreated }: NewNoteCardProps) {
  ...
  function handleStartRecording() {
    ...
    speechRecognition.onerror = (event) => {
      toast.error("Infelizmente ocorreu algum erro durante a gravação.");
      console.error(event);
    };
          ...
          <form className="flex-1 flex flex-col">
            ...
            {isRecording ? (
              ...
              <button
                ...
                disabled={content === ""}
                className="w-full bg-lime-400 py-4 text-center text-sm text-lime-950 outline-none font-medium hover:bg-lime-500 disabled:bg-slate-800 disabled:text-slate-400"
              >
                Salvar nota
              </button>
            )}
